...
class _HomeDashboardScreenState extends State<HomeDashboardScreen> {
  ...
  int _currentActiveTokenIndex = 0;
  final PageController _activeQueueCtrl = PageController();
  final PageController _liveQueueCtrl = PageController();

  void _onActiveQueuePageChanged(int index) {
    if (!mounted) return;
    setState(() => _currentActiveTokenIndex = index);
    if (_activeQueueCtrl.hasClients && _activeQueueCtrl.page?.round() != index) {
      _activeQueueCtrl.animateToPage(index, duration: const Duration(milliseconds: 300), curve: Curves.easeInOut);
    }
    if (_liveQueueCtrl.hasClients && _liveQueueCtrl.page?.round() != index) {
      _liveQueueCtrl.animateToPage(index, duration: const Duration(milliseconds: 300), curve: Curves.easeInOut);
    }
  }

  void _goToPrevToken() {
    if (_currentActiveTokenIndex > 0) {
      _onActiveQueuePageChanged(_currentActiveTokenIndex - 1);
    }
  }

  void _goToNextToken() {
    if (_currentActiveTokenIndex < _activeTokens.length - 1) {
      _onActiveQueuePageChanged(_currentActiveTokenIndex + 1);
    }
  }
  ...
  void dispose() {
    ...
    _activeQueueCtrl.dispose();
    _liveQueueCtrl.dispose();
    super.dispose();
  }
      ...
      children: [
        SizedBox(
          ...
          child: PageView.builder(
            controller: _activeQueueCtrl,
            onPageChanged: _onActiveQueuePageChanged,
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Text(
                        'ACTIVE QUEUE (${index + 1}/${_activeTokens.length})',
                        style: TextStyle(
      ...
      Row(
        ...
        children: [
          IconButton(
            ...
            onPressed: _currentActiveTokenIndex > 0 ? _goToPrevToken : null,
            color: AppTheme.primaryColor,
            disabledColor: AppTheme.textLightColor.withOpacity(0.3),
          ...
          Row(
            mainAxisSize: MainAxisSize.min,
            children: List.generate(_activeTokens.length, (index) {
              final isActive = index == _currentActiveTokenIndex;
          ...
          IconButton(
            ...
            onPressed: _currentActiveTokenIndex < _activeTokens.length - 1 ? _goToNextToken : null,
            color: AppTheme.primaryColor,
            disabledColor: AppTheme.textLightColor.withOpacity(0.3),
            padding: EdgeInsets.zero,
            constraints: const BoxConstraints(),
  ...
  Widget _buildLiveQueueStatusCard(BuildContext context, bool isNarrow) {
    if (_activeTokens.isEmpty) return const SizedBox.shrink();

    return SizedBox(
      height: isNarrow ? 200 : 220, // Fixed height overflow
      child: PageView.builder(
        controller: _liveQueueCtrl,
        onPageChanged: _onActiveQueuePageChanged,
        itemCount: _activeTokens.length,
        itemBuilder: (context, index) {
          final token = _activeTokens[index];
